...
function renderGreenPeppers() {
  ...
  document.querySelectorAll('.green-pepper').forEach(oneG => {
    if(state.greenPeppers){
      oneG.style.visibility = 'visible';
    } else {
      oneG.style.visibility = 'hidden';
    }
  });
}
...
function renderPrice() {
  ...
  if(state.whiteSauce) {
    list.innerHTML += `<li>$1 white sauce</li>`;
    totalPrice += ingredients.whiteSauce.price;
  }
  
  if(state.glutenFreeCrust) {
    list.innerHTML += `<li>$1 gluten-free crust</li>`;
    totalPrice += ingredients.glutenFreeCrust.price;
  }
  
  total.innerHTML = `$ ${totalPrice}`

        //all elements selected add value
/*
    ingredients = {
      pepperoni: { name: 'pepperoni', price: 1 },
*/
